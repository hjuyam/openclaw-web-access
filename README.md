...
## 为什么不使用 agent-browser（与上游差异）

上游 `web-access`（面向 Claude Code）引入 `agent-browser` 的原因主要是：用 accessibility tree 快照节省 token、并用独立 Chrome profile 持久化登录态。

在 OpenClaw 环境里，这些能力由 **原生 `browser` 工具**覆盖：
- 同样支持结构化快照/可交互元素引用（用于低 token 的操作与读取）
- 浏览器实例由 OpenClaw 统一托管，使用 `browser start/stop` 管理生命周期

因此本项目选择：
- **零外置依赖**（不需要额外 npm 全局安装）
- **不做 PID/端口级 kill**（只关闭本次启动的隔离实例，不影响系统其它进程/用户自己的 Chrome）
