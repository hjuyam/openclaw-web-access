...
**选通道：**
- 需要线索/多来源 → `web_search`
- 已知 URL 且静态 → `web_fetch`（Readability 抽正文，默认会过滤导航/侧边栏/广告）
...
## 正文过滤策略（吸收 web-content-fetcher 的思想，但不引入第二套 skill）

目标：**尽可能把网页里的“广告/导航/侧边栏/推荐”变成噪声丢掉，只把正文留下**，同时不引入额外依赖/额外 skill。

### 默认：先用 web_fetch 拿“干净正文”
- `web_fetch` 的抽取器（Readability）通常已经能把 `nav/aside/footer` 等剥离掉。
- 使用时要明确告诉模型：**只输出正文 Markdown**，忽略评论区/推荐阅读。

### web_fetch 结果仍然很吵时：升级 browser，但只抽“正文容器”
当出现这些信号（任一命中即可认为“噪声高/正文抽取失败”）：
- 抽取结果里“目录/站点导航/推荐链接/相关阅读/上一篇下一篇”占比很高
- 正文缺失、只有标题或只剩一堆链接
- **链接密度异常**：每 10 行里有 ≥3 行是纯链接/导航项（或出现大量重复域名）
- **噪声关键词命中**（高频出现）：`推荐`/`相关阅读`/`热门`/`更多`/`关注公众号`/`扫码`/`登录后阅读`/`展开全文`

则用 `browser` 渲染页面后，做一次“正文容器抽取”再返回：
1) `browser.snapshot(refs="aria")`
2) 优先定位 `article` / `main` / 语义最像正文的容器
3) `browser.act(kind="evaluate", fn=...)` 读取该容器的 `innerText`
4) 将 `innerText` 组织成简洁 Markdown（保留标题/小标题/列表/引用，图片用链接占位即可）

> 这一步的关键是：**升级 browser 是为了解决渲染/反爬，但抽取仍然只拿正文容器**，避免把整个页面 DOM/杂讯喂给模型。

### 降级顺序（统一维护）
```
web_search（需要线索时）
web_fetch（静态/可抽取）
browser（动态/反爬/渲染；抽正文容器）
```
...
## Failure Modes
- If a required dependency is missing: clearly explain what is missing and how to install/configure it.
- If an external service is unreachable or auth fails: include the failing endpoint/tool name and the next action.
