...
### "正文抽取是否成功"的噪声判定信号（经验规则）

当你拿到的内容出现以下任一信号，可认为"正文抽取失败/噪声过高"，应升级策略（优先在同一层解决）：
- 抽取结果里"目录/站点导航/推荐链接/相关阅读/上一篇下一篇"占比很高
- 正文缺失、只有标题、或只剩一堆链接
- **链接密度异常**：每 10 行里有 ≥3 行是纯链接/导航项（或出现大量重复域名）
- **噪声关键词高频**：`推荐`/`相关阅读`/`热门`/`更多`/`关注公众号`/`扫码`/`登录后阅读`/`展开全文`

### 升级策略：浏览器层只抽"正文容器"，不要整页灌入上下文

当静态层（Jina/WebFetch）抽取失败或内容很吵时，进入浏览器层后仍应保持"只拿正文"的原则：
- 先定位 `article` / `main` / 语义最像正文的容器
- 只读取该容器的文本（例如 `innerText`），再组织为紧凑 Markdown/要点
- 避免把整页 DOM、导航、推荐区、评论区一并喂给模型导致 token 浪费
